...
namespace _26_Thread_1_
{
    public partial class Play : Form
    {
        ...
        string _stringPlayerName = string.Empty;
        public string StringPlayerName { get => _stringPlayerName; set => _stringPlayerName = value; }
        ...
        {
            InitializeComponent();
            lblPlayerName.Text = StringPlayerName = name;
        }
        public void fThreadStart()
        {
            // _thread = new Thread(new ThreadStart(Run));     //ThreadStart 델리게이트 타입 객체를 생성 후 함수를 넘김

            _thread = new Thread(Run);  //컴파일러에서 델리게이트 객체를 추론해서 생성 후 생성 후 함기를 넘김 (new ThreadStart 생략)
        ...
        private void Run()
        {
            // UI Control이 자신이 만들어낸 Thread가 아닌 다른 Thread에서 접근할 경우 Cross Thread가 발생
            //CheckForIllegalCrossThreadCalls = false;      //Thread 충돌에 대한 예외 처리를 무시
            try
            {
                int ivar = 0;
        ...
        {
            if (_thread.IsAlive)        //Thread가 실행 중일 경우
            {
